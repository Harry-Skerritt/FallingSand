...
public:
    ...
    void init(sf::Vector2f pos, sf::Vector2f size, sf::Color color, std::string text, sf::Font* font = nullptr)
    {
        ...
        centreInButton();  // Correct text centering
    ...
    {
        m_background.setPosition(pos);
        centreInButton();  // Recenter the label
    }
    ...
    {
        m_background.setSize(size);
        centreInButton();  // Recenter after size change
    }
    ...
    void setString(std::string str)
    {
        ...
        centreInButton();  // Recenter the label when text changes
    }
...
private:
    ...
    void centreInButton()
    {
        // Recalculate origin and position each time the label or button changes
        m_label.setOrigin(m_label.getLocalBounds().width / 2.f, m_label.getLocalBounds().height / 2.f);
        m_label.setPosition(
            m_background.getPosition().x + m_background.getSize().x / 2.f,
            m_background.getPosition().y + m_background.getSize().y / 2.f - 4
        );
    }
